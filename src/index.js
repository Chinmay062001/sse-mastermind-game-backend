...
// =========================
const lobbySecrets = new Map(); // lobbyId -> secret
const clients = new Map(); // lobbyId -> clients
    ...
    lobbyClients.forEach(({ res, playerId }) => {
        const serialized = {
            ...
            players: lobby.players.map(p => {
                if (lobby.showAllGuesses || p.id === playerId)
                    return p;
                return { ...p, guesses: [] }; // hide others' guesses
            })
        };
        res.write(`data: ${JSON.stringify({ type: "state", lobby: serialized })}\n\n`);
    });
}
